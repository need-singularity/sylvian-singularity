...
```
  3모드 × 5 trials (Breast Cancer, 8 clones):

  모드              AUROC mean  해석
  ──────────────   ──────────  ──────
  A) Basic         0.845       기본 8-clone 앙상블
  B) Neg.Selection 0.846       음성선택 (+0.001, 미미)
  C) Clonal Exp.   0.845       클론확장 (0.000, 무효과)
  ...
  분석:
    음성선택: AUROC 미세 개선, 하지만 FPR 악화
    클론확장: 완전히 무효과
    → 단순한 면역 비유는 성능 향상에 불충분
    → 면역 시스템의 핵심은 선택/확장이 아니라
       "다양성 생성" 자체 (= mitosis)
```

## 상태: 🟧 부분확인 (구조 동형 확인, 하지만 선택/확장은 무효과)
