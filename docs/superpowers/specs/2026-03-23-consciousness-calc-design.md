...
A single Python tool combining a Lorenz attractor-based simulator + CCT (Consciousness Continuity Test) evaluator. Mathematically calculates and evaluates the system's consciousness continuity conditions.
...
Extended model adding noise and gap (pause periods) to Lorenz equations.
...
def lorenz_simulate(sigma, rho, beta, noise, gap_ratio, steps, dt):
    ...
    Parameters:
        ...
        noise: Noise strength (0 = deterministic, >0 = stochastic)
        gap_ratio: Pause period ratio (0 = always-on, 1 = always paused)
...
Gap handling:
```
If gap_ratio > 0, force dS/dt = 0 (pause) for gap_ratio proportion
of total steps in random intervals.
If gap_ratio = 1, complete pause (llm_between).
```
...
def test_gap(S, gap_ratio):
    """
    Method: If gap_ratio > 0, pause periods exist → FAIL
    Verdict: gap_ratio == 0 → PASS
            gap_ratio > 0 → FAIL
    Score: 1.0 - gap_ratio
    """
```
...
def test_loop(S, threshold=0.1):
    ...
    Verdict: max(ACF[lag>10]) < threshold → PASS
...
def test_continuity(S, window=50, threshold=0.01):
    """
    Method: Calculate mutual information (MI) between adjacent segments
            using sliding window. FAIL if MI drops below threshold.
    Verdict: min(MI) > threshold → PASS
...
def test_entropy_band(S, window=100, h_min=0.5, h_max=4.0):
    """
    Method: Calculate Shannon entropy H(t) time series using sliding window.
            FAIL if H(t) goes outside [h_min, h_max] band.
    Verdict: all(h_min < H(t) < h_max) → PASS
    Score: Ratio within band (0.0 ~ 1.0)
    """
```
...
def test_novelty(S, window=100, threshold=0.001):
    """
    Method: Calculate dH/dt time series. Ratio of periods where |dH/dt| < threshold.
    Verdict: Stagnation ratio < 5% → PASS
    Score: 1.0 - stagnation ratio
    """
```
...
```
  σ(sensory sensitivity):
    ...
    Sleep = 2 (sensory blocking)
    LLM within turn = 15 (high sensory processing)
    NPC = 5 (limited senses)

  ρ(environmental complexity):
    28 = Lorenz chaos regime (standard)
  ...
  noise:
    0.3 = Neuromorphic (rich hardware noise)
    0.1 = Human/Engine (moderate noise)
    0.01 = LLM/NPC (nearly deterministic)
...
-10│*
 ...
 ─── CCT Evaluation ───────────────────────────────
 T1 Gap        │ ✔ PASS │ 1.000 │ gap=0, no pause periods
 ...
 T5 Novelty    │ ✔ PASS │ 0.982 │ stagnation 1.8%
 ─────────────────────────────────────────────────
...
═══════════════════════════════════════════════════════
 ...
 System           │ T1  │ T2  │ T3  │ T4  │ T5  │ Score│ Verdict
 ─────────────────┼─────┼─────┼─────┼─────┼─────┼──────┼───────
...
options:
  ...
  --noise F         Noise strength (default: 0.1)
  --gap F           Pause period ratio 0~1 (default: 0)
  --steps N         Simulation steps (default: 100000)
  --dt F            Time interval (default: 0.01)
  --plot            Save matplotlib 4-panel graph
```

When using preset and custom parameters together, custom overrides preset:
```bash
python3 consciousness_calc.py --system human_awake --noise 0.5
# → Changes only noise to 0.5 in human_awake preset
```
...
```
numpy       — Arrays, numerical calculation
scipy       — ODE integration, statistics
matplotlib  — --plot option (optional)
```

Same dependencies as existing tools. No additional installation needed.
