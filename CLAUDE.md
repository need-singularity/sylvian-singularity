...
```
  ...
  모든 리포: ~/Dev/{리포이름}
  공유 인프라: nexus6/shared/ (심링크: .shared → ../TECS-L/.shared → ../nexus6/shared)
```

## .shared/ 동기화

```
  원본: ~/Dev/nexus6/shared/ (nexus6가 관리)
  심링크: TECS-L/.shared → ../nexus6/shared/
...
```
  ...
  calc/      — 계산기 도구 (203 files, nexus6/shared/calc 심링크)
  tecsrs/    — Rust 계산 엔진 (Monte Carlo, brute-force, ODE)
  math/      — 순수 수학 증명 + 실험
  docs/      — 가설 문서 2,711개 + 논문 스펙
  .shared/   — 크로스 리포 인프라 (→ nexus6/shared)
```
