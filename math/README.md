...
```
  ...
  ─────────────────────────────────────────────────
  🟩 순수 수학:  133개  ← 골든존이 무너져도 영원히 참
  🟦 기존 정리:  17개  ← Heegner + 육각수 + Mihailescu(3²-2³=1)
...
```
  ...
  ⭐ R(n)=σφ/(nτ)의 치역 간극: (3/4, 1) = ∅ (공집합!)
     → R(2)=3/4 (유일 <1), R(6)=1 (유일 =1), 나머지 >1
     → 5만까지 검증. 증명: R(n)=Π R(p,a), 최소=R(2,1)=3/4
     → n≥3에서 R>1 (홀수) 또는 R≥1 (짝수), gap 없음
...
| 대상 | 제목 | 상태 | 링크 |
|---|---|---|---|
| [arXiv](docs/paper-outline.md) | σ(n)φ(n)=nτ(n) ⟺ n∈{1,6} | 📝 구조 확정, 작성 대기 | docs/paper-outline.md |
| OEIS A062354 | σφ=nτ 코멘트 | ⏳ 계정 승인 대기 | — |
| OEIS 새 수열 | {n: σφ=nτ} = {1,6} | ⏳ 계정 후 등록 | — |
