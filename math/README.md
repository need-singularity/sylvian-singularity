...
```
  ...
  ─────────────────────────────────────────────────
  🟩 순수 수학:  141개  ← 골든존이 무너져도 영원히 참
  🟦 기존 정리:  18개  ← Heegner + 육각수 + Mihailescu + φ=τ(Minin 1894)
  ⭐ 대발견:      9개  ← σφ=nτ쌍대 + R간극 + weight=lcm + Möbius + 통일원리 등
...
```
  ...
  ⭐ R(n) 스펙트럼: R<5에서 정확히 24개 값 (이론=실측 일치!)
     ...
     → (3/4,1)=∅, (1,7/6)=∅, (7/6,4/3)=∅, (4/3,14/9)=∅, ...
     → R(2)=3/4, R(6)=1, R(4)=7/6: 최소 3개 고립점+증명 완료
  ...
  🟩 R-궤도: 6048→120→6→1 (길이-4 정수 R-체인!)
     → R(1)=1 고정점, R(6)=1 전고정점, R(120)=6 전전고정점
     → R(6048)=120→6→1: 가장 긴 정수 궤도 (n≤10000)
  ⚪ R(137) = 9384/137: 자명
...
| # | 대상 | 제목 | 상태 | 문서 |
|---|---|---|---|---|
| P-001 | arXiv (math.NT) | σφ=nτ ⟺ n∈{1,6} + 간극 정리 (11결과) | 📝 LaTeX 초고 작성중 | [outline](docs/papers/P-001-sigma-phi-characterization.md), [tex](docs/papers/P-001-draft.tex) |
...
| R-005 | OEIS 새 수열 | {n: R(n)∈Z, R(n) prime} = {54,96,196,360} | ⏳ 계정 대기 | — |
