...
```
  ...
  ─────────────────────────────────────────────────
  🟩 순수 수학:  158개  ← 골든존이 무너져도 영원히 참
  🟦 기존 정리:  18개  ← Heegner + 육각수 + Mihailescu + φ=τ(Minin 1894)
  ⭐ 대발견:     13개  ← σφ=nτ쌍대 + R간극 + R<n증명 + weight=lcm + Möbius + rad(σ)=n + sopfr항등식 + ψ=σ=2n
...
```
  ...
  ⭐ n=T(σ/τ) ⟺ n∈{1,3,6}: 삼각수=평균약수! 47th! (부분 증명!)
     → n=p: (p-1)(p-3)=0→p=3. n=pq: 이차방정식→(2,3)유일
     → n=6: 유일한 합성수 해. T(3)=6=n
