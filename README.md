...
| C44 | 🟨 | 분열 장력 성장 | 25.6→135.4 | experiment_mitosis.py | 10ep 독립 학습 후 장력 5.3x |
| C45 | 🟨 | 분열 vs 설계 반발력장 | -0.11% | experiment_mitosis.py | 분열 97.49% ≈ 설계 97.60% (거의 동등!) |
| C46 | 🟨 | 재결합 효과 | +0.82% | experiment_mitosis.py | 분열 후 합치면 부모보다 좋음 |
| C47 | 🟨 | 형제 인식 비 | 1.65x | experiment_mitosis.py | 형제 R²=88.9% vs 남 81.6% |
...
| C36 | 🟦 | 2^(2/3)·√3 / e | 1.01147 | 산술 | 텍사스 p=0.010, 경계선 — 38/3750 조합이 동등 |
| C37 | ❌ | C7² ≈ 1/3 | 0.309~0.357 | 교차 계산 | 텍사스 p=0.116, 두 실험값이 1/3 양쪽으로 산포 |
| C38 | 🟧 | C12·C21 ≈ 5/3 | 1.676 vs 1.667 | 교차 계산 | 텍사스 p=0.012, 오차 0.54% — 의미 불분명 |
| C41 | 🟧 | C7 ≈ 1/√3 | 0.5766 vs 0.5774 | 교차 도메인 | 텍사스 p=0.033, 오차 0.13% — 오답/정답 비=tan(30°)? |
| C39 | 🟦 | 반발력장 MI 효율 | 70.5% | C24 이론 분석 | 남은 정보 갭의 70.5%를 반발에서 추출 |
| C40 | 🟦 | A-G 공유 MI 하한 | 1.053 nats | 정보 이론 | MI(A)+MI(G)-log(10), 두 극이 45.7% 정보 공유 |
...
| 2026-03-24 | 분열(mitosis) 실험 | 분열≈설계(-0.11%), 재결합+0.82%, 형제인식1.65x | C44-C47 |
